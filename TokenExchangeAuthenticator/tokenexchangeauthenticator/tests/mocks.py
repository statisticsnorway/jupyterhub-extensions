...
# Adapted from
# https://github.com/jupyterhub/oauthenticator/blob/master/oauthenticator/tests/mocks.py
...
def setup_oauth_mock(
    ...
    # Check if token exchange is requested - otherwise fallback to access_token handler
    def access_token_handler(request):
        if token_request_style == 'post':
    ...
    def token_exchange(request, query):
        if 'subject_token' not in query:
            return HTTPResponse(request=request, code=400,
                                reason="No subject_token in token exchange request: url=%s, body=%s" % (
                                    request.url, query)
                                )
        if query['subject_token'][0] not in access_tokens:
            return HTTPResponse(request=request, code=400,
                                reason="subject_token %s not found in access_tokens" % query['subject_token'][0]
                                )
        # check that subject_token in access_tokens
        return {
    ...
    for host in hosts:
        client.add_host(host, [
            (access_token_path, access_token_handler),
            (user_path, get_user),
        ])
